...
class OutboxRelayTest {
    ...
    private final OutboxRelay relay = new OutboxRelay(jdbcTemplate, kafkaTemplate, new ObjectMapper());
    ...
    @Test
    @SuppressWarnings("unchecked")
    void malformedRowDoesNotBlockSubsequentRows() {
        ...
        // headers JSON has no correlationId, so publish() throws before reaching Kafka.
        OutboxRow poison = row(poisonId, "{}");
        OutboxRow healthy = row(healthyId, "{\"correlationId\":\"" + UUID.randomUUID() + "\"}");

        when(jdbcTemplate.query(anyString(), any(RowMapper.class))).thenReturn(List.of(poison, healthy));
